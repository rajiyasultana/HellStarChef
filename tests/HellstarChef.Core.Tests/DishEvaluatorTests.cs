...
namespace HellstarChef.Core.Tests
{
    public class DishEvaluatorTests
    {
        ...
        {
            IEnumerable<Ingredient> set = IngredientRepository.DefaultSet();
            DishEvaluator evaluator = new HellstarChef.Core.Services.DishEvaluator();

            FlavorTotals totals = evaluator.Evaluate(new Dish("Test"), set).Totals;

            Assert.Equal(1.0, totals.Get(Flavor.Savory));
            Assert.Equal(1.0, totals.Get(Flavor.Umami));
            Assert.Equal(2.0, totals.Get(Flavor.Spicy));
        ...
        public void SpicySoup_RuleMatches()
        {
            ...
            Dish ruleDish = new Dish("Spicy Soup", new DishRule[] {
            ...
            Ingredient[] ingredients = new[] { IngredientRepository.FirePaper, IngredientRepository.FirePaper, IngredientRepository.Fish };
            DishEvaluator evaluator = new HellstarChef.Core.Services.DishEvaluator();
            DishEvaluationResult result = evaluator.Evaluate(ruleDish, ingredients);
        ...
        public void InfernoMemoryStew_RuleMatches()
        {
            ...
            Dish dish = new Dish("Inferno Memory Stew", new DishRule[] {
            ...
            DishEvaluator evaluator = new HellstarChef.Core.Services.DishEvaluator();
            DishEvaluationResult result = evaluator.Evaluate(dish, ingredients);
            Assert.True(result.IsMatch);
        }
    }
}
